...
public class PerlinNoise2D {
    ...
    private static BufferedImage image = new BufferedImage(GameApp.WIDTH, GameApp.HEIGHT, BufferedImage.TYPE_INT_RGB);
    public static BufferedImage getNoiseImage(){
        time += 0.01;
        for(int y = 0; y < GameApp.HEIGHT; y++){
            for(int x = 0; x < GameApp.WIDTH; x++){
                double dx = (double) x / GameApp.HEIGHT;
                double dy = (double) y / GameApp.HEIGHT;
                int frequency = 3;
    ...
    public static double[][] getNoiseGrid(int gridSize){
        time += 10;
        double[][] noiseGrid=new double[gridSize][gridSize];
        for(int y = 0; y < gridSize; y++){
            for(int x = 0; x < gridSize; x++){
                double dx = (double) x / gridSize;
                double dy = (double) y / gridSize;
                int frequency = 3;
                double noise = noise((dx * frequency) + time, (dy * frequency) + time);
                noise = (noise ) / 2;//used to be (noise -1)/2
                noiseGrid[x][y]= noise;
            }
        }
        return noiseGrid;
    }
    public static void maskNoise(){
